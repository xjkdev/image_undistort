...
int DepthSemantic::getQueueSize() const {
  int queue_size;
...
DepthSemantic::DepthSemantic(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
    : nh_(nh),
      ...
      semantic_image_sub_(it_, "rect/semantic/image", queue_size_),
      ...
      camera_sync_(CameraSyncPolicy(queue_size_), first_image_sub_,
                   second_image_sub_, semantic_image_sub_, first_camera_info_sub_,
  ...
  camera_sync_.registerCallback(
      boost::bind(&DepthSemantic::camerasCallback, this, _1, _2, _3, _4, _5));
...
void DepthSemantic::fillDisparityFromSide(const cv::Mat& input_disparity,
...
void DepthSemantic::bulidFilledDisparityImage(const cv::Mat& input_disparity,
...
void DepthSemantic::calcPointCloud(
    const cv::Mat& input_disparity, const cv::Mat& left_image, const cv::Mat& semantic_image,
...
void DepthSemantic::calcDisparityImage(
...
bool DepthSemantic::processCameraInfo(
...
bool DepthSemantic::ApproxEq(double A, double B) { return (std::abs(A - B) <= kDelta); }

void DepthSemantic::camerasCallback(
  ...
  cv_bridge::CvImageConstPtr semantic_ptr = cv_bridge::toCvShare(semantic_image_msg);
  calcPointCloud(disparity_ptr->image, left_ptr->image, semantic_ptr->image, baseline, focal_length,
